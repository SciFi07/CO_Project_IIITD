...
class SimGrader(Grader):
	...
	def handleBin(self, genDir, expDir):
		...
		curDir = os.getcwd()
		tests = self.listFiles("tests/assembler_simulator_test_trace/tests/bin/" + genDir)
		tests.sort()
		os.chdir(self.SIM_RUN_DIR)
		
		for test in tests:
			generatedTrace = os.popen("./run < " + "../automatedTesting/tests/assembler_simulator_test_trace/tests/bin/" + genDir + "/" + test).readlines()
			expectedTrace = os.popen("cat " + "../automatedTesting/tests/assembler_simulator_test_trace/tests/traces/" + expDir + "/" + test).readlines()
